...
/** Định dạng tiền tệ đơn giản theo ngôn ngữ user */
...
registry.category("actions").add("hlv_show_panel_noqweb", async (env, action) => {
  ...
  const destroy = () => {
    if (target && target.isConnected) {
      try {
        ...
        console.warn("[HLV] destroy() double call", e);
      }
    }
  };
  ...
  try {
    // Load dữ liệu
    const [order] = await orm.read("sale.order", [resId], [
      "name",
      "partner_id",
      "state",
      "amount_total",
  ...
  // Trả về object có destroy cho ActionManager
  return { destroy };
});

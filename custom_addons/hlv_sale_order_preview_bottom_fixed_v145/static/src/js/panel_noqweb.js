...
function fmtCurrency(env, v) {
  try { return new Intl.NumberFormat((env.services.user?.lang || "vi_VN").replace("_", "-")).format(v ?? 0); }
...
registry.category("actions").add("hlv_show_panel_noqweb", async (env, action) => {
  ...
  const resId = action.params?.res_id ?? ctx.active_id ?? (Array.isArray(ctx.active_ids) && ctx.active_ids[0]);
  ...
  const destroy = () => { if (target?.isConnected) target.remove(); };
  ...
  try {
    ...
    const rows = (lines || []).map(l => `
      <tr>
        ...
        <td class="text-end">${fmtCurrency(env, l.price_subtotal)}</td>
      </tr>`).join("");
    target.querySelector(".hlv-panel-body").innerHTML = `
      <table class="table table-sm table-striped mb-0">
        <thead>
          <tr>
            <th>Sản phẩm</th><th>Số lượng</th><th>Đã giao</th>

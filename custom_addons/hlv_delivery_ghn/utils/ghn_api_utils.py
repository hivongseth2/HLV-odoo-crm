...
class GHNApiUtils:
    ...
    def get_order_detail(self, order_code):
        """
        Get shipping order detail from GHN.
        Endpoint: /shipping-order/detail
        """
        url = f"{self.v2_url}/shipping-order/detail"
        headers = self._get_headers()
        ...
        try:
            ...
            error_msg = res_json.get("message") or f"HTTP {response.status_code}"
            _logger.error("GHN get_order_detail error: %s", response.text)
            return {
                "success": False, 
                "error": error_msg
            }
    ...
    def return_order(self, order_codes):
        """
        Return shipping orders in GHN (Switch status to Return).
        Endpoint: /switch-status/return
        :param order_codes: List of order codes to return (e.g. ["CODE1", "CODE2"])
        """
        url = f"{self.v2_url}/switch-status/return"
        headers = self._get_headers()
        ...
        try:
            ...
            error_msg = res_json.get("message") or f"HTTP {response.status_code}"
            _logger.error("GHN return_order error: %s", response.text)
            return {
                "success": False, 
                "error": error_msg
            }

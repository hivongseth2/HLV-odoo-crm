...
class StockPickingAssignLog(models.Model):
    ...
    def action_assign(self):
        ...
        for picking in self:
            ...
            # picking._fix_ghost_reservations_before_assign()
        ...
        for picking in self:
            _logger.info('[ASSIGN_LOG] ===== action_assign DONE | picking=%s =====', picking.name)

            # Log move lines SAU khi assign
            for move in picking.move_ids.filtered(lambda m: m.state not in ['cancel']):

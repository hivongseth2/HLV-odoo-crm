...
class TestLLMToolSync(common.TransactionCase):
    ...
    def _set_registry(self, registry_dict, xml_keys=None):
        """Helper to mutate class-level registry in-place without triggering setattr hooks."""
        self.LLMTool._tool_registry.clear()
        if registry_dict:
            self.LLMTool._tool_registry.update(registry_dict)
        self.LLMTool._xml_managed_keys.clear()
        if xml_keys:
            self.LLMTool._xml_managed_keys.update(xml_keys)
    ...
    def test_sync_skips_xml_managed_deactivation(self):
        ...
        self._set_registry({}, xml_keys={("res.partner", "xml_method")})
        self.LLMTool.invalidate_model()

...
class ProductTemplate(models.Model):
    ...
    def _compute_combo_selling_price(self):
        ...
        for product in self:
            combo_price, listed_price, tmdt_price, thuongmai_price = product._calculate_combo_price_values(
                pricing_method, discount_pct
            )
            product.computed_combo_selling_price = combo_price

            # Auto-update Odoo price fields if calculated (KHÔNG đụng tới list_price)
            # User request: update x_studio_ga_web, x_studio_ga_hng_nim_yt,
            # x_studio_gia_san_tmdt và x_studio_gi_bn_thng_mi từ BOM
            vals = {}
    ...
    def _calculate_combo_price_values(self, pricing_method='sum_combo_price', discount_pct=0.0):
        """
        Tính giá combo (bán, niêm yết, sàn TMĐT, thương mại) dựa trên BOM

        Returns:
            tuple: (selling_price, listed_price, tmdt_price, thuongmai_price)
        """
        self.ensure_one()
        ...
        return total_selling_price, total_listed_price, total_tmdt_price, total_thuongmai_price

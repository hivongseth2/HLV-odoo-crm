...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function handlePackageBarcode(packageBarcode) {
    ...
    if (completedItems.length === 0) {
      toast.warn("Không có sản phẩm nào được quét để tạo kiện", { ms: 2500 });
      playError();
      setFocus();
      return;
    }
...
// Quantity adjustment function for modern modal
...
// Enhanced modern modal opening with animation
function openModernPackageEditModal(packageId) {
  const modal = document.getElementById('packageEditModal');
  modal.classList.add('active');

  // Add staggered entrance animations
  const modalContainer = modal.querySelector('.modern-modal-container');
  const modalHeader = modal.querySelector('.modern-modal-header');
  const modalBody = modal.querySelector('.modern-modal-body');
  const modalFooter = modal.querySelector('.modern-modal-footer');

  // Reset animations
  modalContainer.style.animation = 'none';
  modalHeader.style.animation = 'none';
  modalBody.style.animation = 'none';
  modalFooter.style.animation = 'none';

  // Force reflow
  void modalContainer.offsetWidth;

  // Apply staggered animations
  modalContainer.style.animation = 'modalSlideUp 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)';
  modalHeader.style.animation = 'modalFadeIn 0.6s ease-out 0.1s both';
  modalBody.style.animation = 'modalFadeIn 0.7s ease-out 0.2s both';
  modalFooter.style.animation = 'modalFadeIn 0.8s ease-out 0.3s both';

  // Load package data
  loadPackageDataModern(packageId);
}

// Enhanced modern modal closing with animation
function closeModernPackageEditModal() {
  const modal = document.getElementById('packageEditModal');
  const modalContainer = modal.querySelector('.modern-modal-container');

  // Add closing animation
  modalContainer.style.animation = 'modalSlideDown 0.4s cubic-bezier(0.34, 1.56, 0.64, 1) both';
...
// Load package data with modern UI updates
async function loadPackageDataModern(packageId) {
  try {
    const response = await fetch(`/pack_scan/get_package_data/${packageId}`, {
      method: 'GET',
      headers: {
        'X-Requested-With': 'XMLHttpRequest'
      }
    ...
    // Update modal title
    document.getElementById('modalPackageName').textContent = data.package_name;

    // Update items counter with animation
    const counterNumber = document.querySelector('.counter-number');
    const oldCount = parseInt(counterNumber.textContent);
    const newCount = data.items.length;

    if (oldCount !== newCount) {
      // Animate counter change
      animateValue(counterNumber, oldCount, newCount, 500);
    }

    // Populate items grid with modern cards
    populateModernItemsGrid(data.items);

    // Populate product select
    await populateModernProductSelect(data.available_products);

  } catch (error) {
    console.error('Error loading package data:', error);
    toast.error('Không thể tải dữ liệu gói hàng');
  }
}
...
// Populate modern items grid
function populateModernItemsGrid(items) {
  const grid = document.getElementById('packageItemsList');
  const emptyState = document.getElementById('emptyStateItems');
  ...
  grid.innerHTML = items.map((item, index) => `
    <div class="modern-item-card" style="animation: modalFadeIn 0.5s ease-out ${0.1 * index}s both">
      <div class="modern-item-info">
        <span class="modern-item-name">${item.product_name}</span>
        <span class="modern-item-sku">${item.product_sku || item.product_code || ''}</span>
      </div>
      <div class="modern-item-controls">
        <div class="modern-qty-control">
          <button class="modern-qty-btn" onclick="adjustItemQty(${item.move_line_id}, -1)">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <line x1="5" y1="12" x2="19" y2="12"></line>
            </svg>
          </button>
          <div class="modern-qty-display">${item.quantity}</div>
          <button class="modern-qty-btn" onclick="adjustItemQty(${item.move_line_id}, 1)">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <button class="modern-item-remove" onclick="removeItemFromPackage(${item.move_line_id})">
...
// Populate modern product select
async function populateModernProductSelect(products) {
  ...
  products.forEach(product => {
    const option = document.createElement('option');
    option.value = product.move_line_id;
    option.textContent = `${product.product_name} ${product.product_sku ? `(${product.product_sku})` : ''}`;
    select.appendChild(option);
  });
}

// Adjust item quantity with modern feedback
async function adjustItemQty(moveLineId, delta) {
  try {
    const response = await fetch('/pack_scan/adjust_item_qty', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Requested-With': 'XMLHttpRequest'
    ...
    const itemCard = document.querySelector(`[onclick*="adjustItemQty(${moveLineId}"]`).closest('.modern-item-card');
    itemCard.style.animation = 'none';
    void itemCard.offsetWidth;
    itemCard.style.animation = 'successPulse 0.6s ease';

    // Reload package data
    const currentPackageId = document.getElementById('modalPackageName').textContent;
    await loadPackageDataModern(currentPackageId);
...
// Remove item from package with modern confirmation
async function removeItemFromPackage(moveLineId) {
  // Modern confirmation dialog
  if (!confirmModern('Xóa sản phẩm này khỏi gói hàng?')) {
    return;
  }

  try {
    const response = await fetch('/pack_scan/remove_item_from_package', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-Requested-With': 'XMLHttpRequest'
    ...
    const itemCard = document.querySelector(`[onclick*="removeItemFromPackage(${moveLineId}"]`).closest('.modern-item-card');
    itemCard.style.animation = 'slideOutRight 0.4s ease both';

    setTimeout(() => {
      // Reload package data
      const currentPackageId = document.getElementById('modalPackageName').textContent;
      loadPackageDataModern(currentPackageId);
    }, 400);

    toast.success('Đã xóa sản phẩm khỏi gói hàng');
...
// Modern confirmation dialog
function confirmModern(message) {
...
// Override existing functions to use modern versions
const originalOpenPackageEditModal = window.openPackageEditModal;
const originalClosePackageEditModal = window.closePackageEditModal;

window.openPackageEditModal = function(event) {
  event.preventDefault();
  event.stopPropagation();
  const packageId = event.currentTarget.dataset.packageId;
  openModernPackageEditModal(packageId);
};

window.closePackageEditModal = closeModernPackageEditModal;

// Initialize modal with proper styling
document.addEventListener('DOMContentLoaded', function() {
  // Add click-outside-to-close functionality
  document.getElementById('packageEditModal')?.addEventListener('click', function(event) {
    if (event.target === this) {
      closeModernPackageEditModal();
    }
  });

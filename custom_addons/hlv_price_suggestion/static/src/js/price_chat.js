...
class PriceChatAction extends Component {
    ...
    async onSendMessage() {
        ...
        // Thêm placeholder AI đang trả lời
        ...
        try {
            ...
            const result = await this.orm.call(
                "price.chat.session",
                "rpc_send_message",
                [this.state.currentSessionId, text],
            );

            // Xóa loading placeholder, thêm tin nhắn thật
    ...
    onExportExcel() {
        ...
        const url = `/price_chat/export_excel/${this.state.currentSessionId}`;
    ...
    /**
     * Convert markdown-like text to simple HTML for display.
     * Handles: **bold**, headers (#, ##), bullet lists, newlines.
     */
    formatContent(text) {
        if (!text) return "";
        let html = text;
        ...
        // Bullet lists
        html = html.replace(/^[-•] (.+)$/gm, '<div class="pc-li">• $1</div>');
        // Numbered lists

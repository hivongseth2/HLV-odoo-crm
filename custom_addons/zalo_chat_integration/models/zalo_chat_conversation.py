...
class ZaloChatConversation(models.Model):
    ...
    def _get_or_create_partner(self, zalo_user_id, user_info):
        ...
        if not partner:
            # Create new partner
            name = user_info.get('display_name') or user_info.get('user_name') or f'Zalo User {zalo_user_id[-4:]}'
            partner_vals = {
                'name': name,
                'zalo_user_id': zalo_user_id,
            }
            ...
            if user_info.get('user_id_by_app'):
                partner_vals['phone'] = user_info.get('user_id_by_app')
            
            partner = Partner.create(partner_vals)
            _logger.info(f'Created partner for Zalo user: {name}')

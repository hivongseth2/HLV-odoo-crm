...
function startSalePlanMentionListener(){loadPublicMentionAliases();loadCurrentMentionAlias().then(function(){if(!getCurrentMentionAliases().length)return;pollMentionEvents();setInterval(function(){if(!document.hidden)pollMentionEvents();},60000);});}

...
function mentionAliasMatchScore(row,term){if(!term)return 0;var alias=normalizeMentionAlias(row&&row.alias),display=normalizeMentionAlias(row&&row.display_alias),name=normalizeMentionAlias(row&&row.user_name);if(alias===term||display===term)return 0;if(alias.indexOf(term)===0||display.indexOf(term)===0)return 1;if(name.indexOf(term)===0)return 2;if(name.indexOf(term)>0)return 3;return -1;}

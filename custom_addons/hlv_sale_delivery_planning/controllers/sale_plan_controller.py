...
_SKIP_MSG_RE = re.compile(
    ...
    r'|🔗 Tự động gộp hóa đơn trùng'
    r'|🔗 Tự động phát hiện xuất HĐ chung'
    r'|Tình trạng xuất hóa đơn MISA:'
    r'|Lệch tiền với MISA:'
    r'|Kiểm tra hóa đơn MISA thất bại'
    r'|MISA sa_invoice_request lỗi'
    r'|🖨️'
    r'|👤'
    r'|🔄'
...
.public-mention-suggest{position:fixed;z-index:3200;background:#fff;border:1px solid #c7d2fe;border-radius:6px;box-shadow:0 12px 30px rgba(15,23,42,.16);max-height:360px;overflow-y:auto;display:none}
...
// Xep hang goi y @mention: alias la cai nguoi dung go ra, con user_name chi la thong tin phu
// va duoc match kieu substring (rong hon nhieu) - neu khong cham diem thi mot alias trung khop
// hoan toan co the bi day xuong duoi mot alias chi dinh vi ten user co chua tu do o giua.
// Tra ve -1 = khong khop; so cang nho cang uu tien. Term rong => moi alias deu diem 0.
/* Bo dấu để gõ không dấu vẫn tìm ra biệt danh có dấu (gõ "ha" ra "Hạnh BC").
   Đ không tách được bằng NFD nên phải thay tay. Chỉ dùng khi so khớp tìm kiếm;
   normalizeMentionAlias vẫn giữ nguyên dấu vì đó là khoá đối chiếu với alias của server. */
function foldMentionText(v){return normalizeMentionAlias(v).normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/\u0111/g,'d');}
function mentionAliasMatchScore(row,term){var t=foldMentionText(term);if(!t)return 0;var alias=foldMentionText(row&&row.alias),display=foldMentionText(row&&row.display_alias),name=foldMentionText(row&&row.user_name);if(alias===t||display===t)return 0;if(alias.indexOf(t)===0||display.indexOf(t)===0)return 1;if(name.indexOf(t)===0)return 2;if(name.indexOf(t)>0)return 3;if(alias.indexOf(t)>0||display.indexOf(t)>0)return 4;return -1;}
function renderPublicMentionSuggest(input){var box=$('dr-mention-suggest');if(!box)return;var q=currentPublicMentionQuery(input);if(!q){box.classList.remove('open');box.innerHTML='';return;}var items=_mentionAliases.map(function(a){return {row:a,score:mentionAliasMatchScore(a,q.term)};}).filter(function(x){return x.score>=0;}).sort(function(x,y){return x.score-y.score||normalizeMentionAlias(x.row.alias).localeCompare(normalizeMentionAlias(y.row.alias));}).map(function(x){return x.row;}).slice(0,30);if(!items.length){box.classList.remove('open');box.innerHTML='';return;}_mentionActiveIndex=Math.min(Math.max(_mentionActiveIndex,0),items.length-1);box.innerHTML=items.map(function(a,i){var label=a.display_alias||a.alias;return '<div class="public-mention-suggest-item '+(i===_mentionActiveIndex?'active':'')+'" data-alias="'+esc(label)+'"><strong>@'+esc(label)+'</strong><small>'+esc(a.user_name||'')+'</small></div>';}).join('');box.classList.add('open');positionPublicMentionSuggest(input,box);}
/* Hộp gợi ý @mention nằm trong .msg-section (overflow:hidden) và #drawer (overflow-y:auto)
   nên position:absolute sẽ bị cắt — dùng position:fixed bám theo toạ độ ô nhập để không phụ thuộc cha. */
function positionPublicMentionSuggest(input,box){if(!input||!box)return;var r=input.getBoundingClientRect();var below=window.innerHeight-r.bottom-12,above=r.top-12;var up=below<160&&above>below;var maxH=Math.max(120,Math.min(360,up?above:below));box.style.left=r.left+'px';box.style.width=r.width+'px';box.style.maxHeight=maxH+'px';if(up){box.style.top='auto';box.style.bottom=(window.innerHeight-r.top+4)+'px';}else{box.style.bottom='auto';box.style.top=(r.bottom+4)+'px';}}
function repositionPublicMentionSuggest(){var box=$('dr-mention-suggest');if(!box||!box.classList.contains('open'))return;positionPublicMentionSuggest($('dr-msg-input'),box);}
window.addEventListener('resize',repositionPublicMentionSuggest);
document.addEventListener('scroll',repositionPublicMentionSuggest,true);

  ...
  for user in users:
    for alias in _split_mention_aliases(user.x_sale_plan_mention_names):
      if alias in seen:
        continue
      seen.add(alias)
      rows.append({'alias': alias, 'user_id': user.id, 'user_name': user.name or ''})
...
def _push_public_mention_event(env, so, body, author_name=''):
  ...
  mentioned = set(_extract_mention_aliases(body))
  if not mentioned:
    return None
  valid_aliases = {row['alias'] for row in _get_sale_plan_alias_rows(env)}
  matched = sorted(mentioned.intersection(valid_aliases))
  if not matched:
    return None
  ...
  <div class="collapse navbar-collapse" id="nav1">
    <ul class="navbar-nav ms-auto">
      ...
      <li class="nav-item ms-lg-2 mention-noti-wrap">
        <button id="mention-noti-button" type="button" title="Thong bao tag"><i class="fa fa-bell"></i><span id="mention-noti-count">0</span></button>
        <div id="mention-noti-panel" aria-live="polite">
          <div class="mention-noti-head"><span>Thong bao tag</span><span class="mention-noti-actions"><button id="mention-noti-read-all" type="button">Da doc het</button><button id="mention-noti-clear" type="button">Xoa</button></span></div>
...
function mentionNotiStorageKey(){var aliases=getCurrentMentionAliases();return 'hlv_sale_plan_mentions_'+(aliases.length?aliases.join('_'):'anonymous');}
...
function startSalePlanMentionListener(){loadPublicMentionAliases();loadCurrentMentionAlias().then(function(){if(!getCurrentMentionAliases().length)return;pollMentionEvents();var busLast=0,stopped=false;window.addEventListener('beforeunload',function(){stopped=true;});function loop(){if(stopped)return;if(document.hidden){setTimeout(loop,15000);return;}fetch('/longpolling/poll',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({channels:['sale_plan_public_channel'],last:busLast})}).then(function(r){return r.ok?r.json():null;}).then(function(data){var notifications=(data&&data.result&&data.result.notifications)||data&&data.notifications||[];notifications.forEach(function(n){busLast=Math.max(busLast,n.id||busLast);var payload=n.payload||n.message||n[2]||{};var typ=n.type||n[1];if(typ==='sale_plan_mention'||payload.type==='sale_plan_mention')handleMentionEvent(payload);});setTimeout(loop,0);}).catch(function(){setTimeout(loop,15000);});}loop();});}

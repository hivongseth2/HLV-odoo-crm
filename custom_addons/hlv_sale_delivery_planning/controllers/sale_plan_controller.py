...
#mention-noti-button,#mention-browser-noti-button{border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.08);color:#fff;border-radius:4px;padding:5px 10px;font-size:.78rem;font-weight:700;line-height:1;display:inline-flex;align-items:center;gap:6px}
#mention-noti-button.has-unread{background:#4f46e5;border-color:#818cf8;color:#fff}
#mention-browser-noti-button.enabled{background:#16a34a;border-color:#86efac;color:#fff}
#mention-browser-noti-button.blocked{background:#7f1d1d;border-color:#fecaca;color:#fff}
...
function requestBrowserNotifications(){if(!browserNotificationsSupported()){updateBrowserNotiButton();return Promise.resolve('unsupported');}return fetch('/api/sale_plan/webpush_config',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',method:'call',params:{}})}).then(function(r){return r.json();}).then(function(j){var cfg=j.result||{};if(!cfg.enabled||!cfg.public_key)throw new Error('webpush_not_configured');var permissionPromise=Notification.permission==='granted'?Promise.resolve('granted'):Notification.requestPermission();return permissionPromise.then(function(p){if(p!=='granted')throw new Error('permission_'+p);return subscribeSalePlanPush(cfg.public_key,false).catch(function(firstErr){console.warn('sale plan webpush first subscribe failed, retrying clean',firstErr);return subscribeSalePlanPush(cfg.public_key,true);});}).then(function(sub){return fetch('/api/sale_plan/webpush_subscribe',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',method:'call',params:{subscription:sub.toJSON(),backend_messages:false}})});}).then(function(){updateBrowserNotiButton();return 'granted';});}).catch(function(e){console.warn('sale plan webpush subscribe failed',e);updateBrowserNotiButton();var msg=(e&&e.name==='AbortError')?'Không kết nối được push service của trình duyệt. Thử Chrome/Edge khác, tắt VPN/proxy/adblock, hoặc kiểm tra quyền notification của site.':('Không bật được Web Push: '+(e&&e.message?e.message:e));alert(msg);return 'error';});}

...
- **Strict Viewport Height (`100dvh` / `100vh`)**: Giải quyết triệt để lỗi tràn trang và kẹt cuộn của Odoo Web Client bằng cách khống chế tuyệt đối chiều cao ứng dụng Barcode bằng `100dvh` (`100vh` fallback) và `overflow: hidden`. Cho phép các container danh sách sản phẩm (`.picking-scroll-area`, `.subview-scroll-area`) tự động cuộn nội bộ trơn tru, đồng thời ghim chân trang `.picking-footer` và `.subview-footer` cố định ở đáy màn hình.
- **Adaptive Responsive Bố cục**:
  - *Giao diện Mobile*: Dải camera ngang persistent được nâng chiều cao lên `160px` giúp tăng góc quét và căn chỉnh mã vạch dễ dàng hơn.
  - *Giao diện Desktop (Màn hình lớn ≥ 992px)*: Tự động chuyển đổi `shipper-container` thành CSS Grid 2 cột. Cột trái cố định rộng `400px` hiển thị camera widget kích thước lớn (cao `280px`), cột phải là nội dung chi tiết co giãn linh hoạt (`flex: 1`). Màn hình trang chủ Dashboard tự động sắp xếp 2 cột đối xứng và cân đối.
...
- Bố cục lưới responsive được xử lý hoàn toàn qua CSS Grid/Flexbox thông minh bằng việc kế thừa thuộc tính `:attf-class` của `shipper-container` (thêm `main-view` hoặc `sub-view`) nên không cần can thiệp cấu trúc JS/XML khi mở rộng màn hình mới.

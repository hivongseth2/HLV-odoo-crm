...
def _uses_qty_scanned_progress(picking):
    if not picking or not picking.exists() or _is_return_picking(picking):
        return False
    return _is_pick_picking(picking) or _is_putaway_picking(picking)
...
class HLVMobileBarcodeController(http.Controller):
    ...
    def get_picking_data(self, picking_id):
        ...
        if picking.source_transfer_id and picking.state in ['draft', 'waiting', 'confirmed', 'partially_available', 'assigned']:
            for move in picking.sudo().move_ids:
                if move.state in ['draft', 'waiting', 'confirmed', 'partially_available', 'assigned'] and move.product_uom_qty == 0:
        ...
        # Step 2 must be assigned before mobile scanning so package scans can
        # reuse the generated move lines instead of creating replacement lines.
        has_mobile_progress = any(ml.qty_scanned > 0 for ml in picking.move_line_ids)
        if (
            picking.source_transfer_id
            and not has_mobile_progress
            and picking.state in ['waiting', 'confirmed', 'partially_available']
        ):
            picking.sudo().with_context(skip_unreserve_wizard=True).action_assign()
    ...
    def process_barcode(self, picking_id, barcode, destination_location_id=None, last_product_id=None, location_mode=None, is_multi_location=False, preferred_move_line_id=None):
        ...
        if package:
            ...
            # Step 2 package contents come from the source transfer. Only
            # update existing assigned lines; never create replacement lines.
            if picking.source_transfer_id:
                source_package_lines = picking.source_transfer_id.move_line_ids.filtered(
                    lambda ml: ml.result_package_id == package and ml.quantity > 0
                )
                if not source_package_lines:
                    return {
                        'error': _(
                            'Kiện hàng "%s" không có dữ liệu sản phẩm từ phiếu nguồn của phiếu Bước 2.',
                            package.name,
                        )
                    }

                package_qty_by_product = {}
                source_moves_by_product = {}
                for source_line in source_package_lines:
                    product_id = source_line.product_id.id
                    package_qty_by_product[product_id] = package_qty_by_product.get(product_id, 0.0) + source_line.quantity
                    source_moves_by_product.setdefault(product_id, request.env['stock.move'].browse())
                    source_moves_by_product[product_id] |= source_line.move_id

                matched_lines = []
                used_line_ids = set()
                for product_id, package_qty in package_qty_by_product.items():
                    product = request.env['product.product'].browse(product_id)
                    product_lines = picking.move_line_ids.filtered(
                        lambda ml: (
                            ml.product_id == product
                            and ml.state not in ['done', 'cancel']
                            and ml.id not in used_line_ids
                        )
                    ).sorted('id')

                    target_line = product_lines.filtered(
                        lambda ml: ml.package_id == package or ml.result_package_id == package
                    )[:1]
                    if not target_line:
                        source_moves = source_moves_by_product[product_id]
                        target_line = product_lines.filtered(
                            lambda ml: (
                                not ml.package_id
                                and not ml.result_package_id
                                and ml.qty_scanned <= 0
                                and bool(ml.move_id.move_orig_ids & source_moves)
                            )
                        )[:1]
                    if not target_line:
                        target_line = product_lines.filtered(
                    ...
                    if not target_line:
                        return {
                            'error': _(
                                'Không tìm thấy dòng đã Assign phù hợp cho sản phẩm "%s" trong kiện "%s" trên phiếu Bước 2. Vui lòng kiểm tra lại dữ liệu Assign.',
                    ...
                    used_line_ids.add(target_line.id)
                    matched_lines.append((target_line, package_qty))

                processed_lines = []
                for target_line, package_qty in matched_lines:
                    target_line.write({
                        'package_id': package.id,
                        'result_package_id': package.id,
                        'qty_scanned': package_qty,
                    })
                    processed_lines.append(f"{package_qty} x {target_line.product_id.display_name}")

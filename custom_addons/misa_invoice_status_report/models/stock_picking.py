...
class StockPickingMisaInvoiceStatus(models.Model):
    ...
    def _misa_invoice_picking_to_row(self, picking, today):
        ...
        return {
            ...
            # Lý do lệch đã tính sẵn (xem _misa_invoice_refresh_gap_summary) — CHỈ tính ở phiếu
            # đại diện (diff_source), giống hệt amount_diff/amount_mismatch ở trên. Trước đây
            # 2 field này chỉ được gắn thêm riêng trong get_misa_invoice_discrepancy (chỉ dùng ở
            # dashboard nội bộ) — đưa vào ĐÂY (hàm build row DÙNG CHUNG) để trang public
            # /misa_sale_status cũng thấy được giải thích, không chỉ mỗi con số trơ trụi.
            'gap_summary': diff_source.misa_invoice_gap_summary or '',
            'gap_resolved': diff_source.misa_invoice_gap_resolved,
        }
    ...
    # ==================== Trang /misa_sale_status (misa_invoice_public_controller) ====================

...
class StockPickingMisaInvoiceDashboardData(models.Model):
    ...
    def _misa_invoice_exact_invoiced_total_for_saler(self, misa_domain, saler_value):
        """Tổng tiền ĐÃ xuất HĐ, dùng cho get_misa_invoice_reconciliation_totals khi có
        saler_code — tính ĐÚNG theo TỪNG ĐƠN HÀNG thuộc saler_value (order.x_studio_misa_saler_code),
        KHÔNG cộng theo misa_invoice_effective_amount của phiếu đại diện như cách cũ.

        Lý do: 1 đề nghị xuất HĐ có thể gộp chung khách hàng của NHIỀU sale khác nhau — cách cũ
        (read_group misa_invoice_effective_amount theo picking.misa_invoice_saler_code) sẽ gán
        TRỌN VẸN tiền hóa đơn của cả nhóm cho ĐÚNG 1 saler (saler của phiếu đại diện, vốn chỉ là
        SALER CỦA ĐƠN ĐẦU TIÊN tìm thấy trong nhóm phiếu — xem _compute_misa_invoice_saler_code),
        dù tiền đó thực chất thuộc về nhiều đơn của nhiều saler khác nhau.

        Ở đây tra ĐÚNG theo order_code (misa_invoice_exact_invoiced_amount — đã tính qua API
        sống, đọc dòng hàng MISA thật, xem sale_order.py/_misa_invoice_reconcile_order_coverage)
        cho TỪNG đơn hàng CỦA ĐÚNG saler_value, không quan tâm phiếu đại diện thuộc saler nào.
        Ưu tiên đọc số đã lưu sẵn; đơn nào CHƯA từng tính thì chủ động gọi API ngay (và lưu lại
        cho lần sau, không phải tính lại) — hạn chế tối đa việc ước lượng.

        GIỚI HẠN ĐÃ BIẾT (giống mọi chỗ dùng misa_invoice_exact_*): số này tính trên TOÀN BỘ
        lịch sử đơn hàng, KHÔNG cắt theo date_from/date_to đang lọc — nếu đơn có phần đã xuất
        kho/xuất HĐ từ TRƯỚC khoảng ngày đang xem, vẫn tính đủ, có thể lệch nhẹ khi lọc ngày hẹp
        (đã ghi rõ trong misa_invoice_reconciliation_gap_explain, không xử lý thêm ở đây)."""
        Picking = self.sudo()
        SaleOrder = self.env['sale.order'].sudo()
        in_scope_picking_ids = set(Picking.search(misa_domain).ids)
        if not in_scope_picking_ids:
            return 0.0
        candidate_orders = SaleOrder.search([('x_studio_misa_saler_code', '=', saler_value)])
        relevant_orders = candidate_orders.filtered(
            lambda o: set(o.misa_invoice_picking_ids.ids) & in_scope_picking_ids
        )
        total = 0.0
        for order in relevant_orders:
            if not order.misa_invoice_exact_checked_at:
                picking = order.misa_invoice_picking_ids[:1]
                if picking:
                    try:
                        picking._misa_invoice_reconcile_order_coverage()
                    except Exception:
                        pass
            total += order.misa_invoice_exact_invoiced_amount or 0.0
        return total

            ...
            if any(line.qty < 0 for line in order.lines):
                ...
                for picking in order.picking_ids:
                    ...
                    for move in picking.move_ids:
                        ...
                        # Odoo 18: move.pos_line_id hoặc tìm qua product
                        ...
                        if line and line[0].refunded_orderline_id:
                            ...
                            # Tìm vị trí xuất kho chi tiết ban đầu
                            original_move_lines = original_order.picking_ids.move_line_ids.filtered(
                                ...
                                and ml.picking_id.picking_type_id.code == 'outgoing'
                            )
                            
                            if original_move_lines:
                                target_location = original_move_lines[0].location_id
                                if target_location:
                                    _logger.info("POS Return: Correcting move %s destination to %s", 
                                               move.product_id.name, target_location.complete_name)
                                    # Cập nhật cả Move và các Move Line (nếu có)
